    ...
    <div class="content">
        ...
        <div class="block block-rounded p-3">
            ...
            <div class="block-content block-content-full">
                <form action="{{ route($adminPrefix . '.' . $khoaHocPrefix . '.update', $khoaHoc->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-xl-8">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <select class="form-select @error('level') is-invalid @enderror"
                                        ...
                                        <option value="beginner" {{ old('level', $khoaHoc->level) == 'beginner' ? 'selected' : '' }}>Cơ Bản</option>
                                        <option value="intermediate" {{ old('level', $khoaHoc->level) == 'intermediate' ? 'selected' : '' }}>Trung Cấp</option>
                                        <option value="advanced" {{ old('level', $khoaHoc->level) == 'advanced' ? 'selected' : '' }}>Nâng Cao</option>
                                    </select>
                                    @error('level')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-4">
                                    ...
                                    <select class="form-select @error('coach_id') is-invalid @enderror"
                                        ...
                                        @foreach($coaches ?? [] as $id => $name)
                                            <option value="{{ $id }}" {{ (string) old('coach_id', $khoaHoc->coach_id) === (string) $id ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select @error('branch_id') is-invalid @enderror"
                                    ...
                                    @foreach($branches ?? [] as $id => $name)
                                        <option value="{{ $id }}" {{ (string) old('branch_id', $khoaHoc->branch_id) === (string) $id ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                        ...
                                        rows="4"
                                        placeholder="Nhập mô tả khóa học">{{ old('description', $khoaHoc->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-4">
                                    ...
                                    <select class="form-select @error('club_id') is-invalid @enderror"
                                        ...
                                        @foreach($clubs ?? [] as $id => $name)
                                            <option value="{{ $id }}" {{ (string) old('club_id', $khoaHoc->club_id ?? '') === (string) $id ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4 mb-4">
                                    ...
                                    <select class="form-select @error('quarter') is-invalid @enderror"
                                        ...
                                        <option value="Q1" {{ old('quarter', $khoaHoc->quarter) == 'Q1' ? 'selected' : '' }}>Quý 1</option>
                                        <option value="Q2" {{ old('quarter', $khoaHoc->quarter) == 'Q2' ? 'selected' : '' }}>Quý 2</option>
                                        <option value="Q3" {{ old('quarter', $khoaHoc->quarter) == 'Q3' ? 'selected' : '' }}>Quý 3</option>
                                        <option value="Q4" {{ old('quarter', $khoaHoc->quarter) == 'Q4' ? 'selected' : '' }}>Quý 4</option>
                                    </select>
                                    @error('quarter')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-4 mb-4">
                                    ...
                                    <input type="number" class="form-control @error('year') is-invalid @enderror"
                                        id="year" name="year"
                                        value="{{ old('year', $khoaHoc->year ?? date('Y')) }}"
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-4">
                                    ...
                                    <input type="date" class="form-control @error('start_date') is-invalid @enderror"
                                        ...
                                        value="{{ old('start_date', optional($khoaHoc->start_date)->format('Y-m-d')) }}">
                                    @error('start_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-4">
                                    ...
                                    <input type="date" class="form-control @error('end_date') is-invalid @enderror"
                                        ...
                                        value="{{ old('end_date', optional($khoaHoc->end_date)->format('Y-m-d')) }}">
                                    @error('end_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-4">
                                    ...
                                    <input type="number" class="form-control @error('current_students') is-invalid @enderror"
                                        ...
                                        value="{{ old('current_students', $khoaHoc->current_students ?? 0) }}"
                                        min="0" placeholder="0">
                        ...
                        <div class="col-xl-4">
                            <div class="block block-rounded bg-body-light mb-0 p-3">
                                <div class="block-content py-3">
                                    ...
                                    <p class="fw-semibold mb-4">#{{ $khoaHoc->id }}</p>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="url" class="form-control @error('image_url') is-invalid @enderror"
                                            ...
                                            value="{{ old('image_url', $khoaHoc->image_url) }}"
                                            placeholder="https://…">
                                        @error('image_url')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select class="form-select @error('is_active') is-invalid @enderror"
                                            ...
                                            <option value="1" {{ old('is_active', $khoaHoc->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Hoạt Động</option>
                                            <option value="0" {{ old('is_active', $khoaHoc->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Không Hoạt Động</option>
                                        </select>
                                        @error('is_active')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="border-top pt-3 mb-0">
                                        <a href="{{ route($adminPrefix . '.' . $khoaHocPrefix . '.index') }}" class="btn btn-sm btn-outline-secondary w-100 mb-2">
                                            <i class="ti ti-list me-1"></i>Danh sách khóa học
                                        </a>
                                        <a href="{{ route($adminPrefix . '.' . $khoaHocPrefix . '.show', $khoaHoc->id) }}" class="btn btn-sm btn-outline-primary w-100">
