...
<div class="page-header">
    <div class="row align-items-center">
        <div class="col">
            <h3 class="page-title mb-1">Cập nhật chi nhánh</h3>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route($adminPrefix . '.' . $chiNhanhPrefix . '.update', $chiNhanh->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="club_id" class="form-select @error('club_id') is-invalid @enderror" required>
                        ...
                        @foreach(($clubs ?? []) as $id => $name)
                            <option value="{{ $id }}" {{ (string)old('club_id', $chiNhanh->club_id) === (string)$id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    <div class="form-check">
                        <input
                            ...
                            value="1"
                            id="is_active"
                            {{ old('is_active', (int)$chiNhanh->is_active) ? 'checked' : '' }}
                        >
