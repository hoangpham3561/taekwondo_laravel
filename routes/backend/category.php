...
Route::group(['prefix' => $adminPrefix], function () use ($adminPrefix) {
    ...
    Route::middleware(['isAdmin'])->group(function () use ($adminPrefix) {
        ...
        Route::middleware(['role:super_admin'])->group(function () use ($adminPrefix) {
            /** CATEGORY NEWS **/
            $categoryPrefix = Config::get('core.routes.category.prefix');
            $baseHelper = new BaseHelper();
            Route::resource($categoryPrefix, CategoryController::class, $baseHelper->customRouteName($adminPrefix, $categoryPrefix))
                ->except(['show']);
        });
    });
});
